...
//--------------------------------------------------------------------------
// Modified in 2022 by Joshua Goodwin based upon the uncanny eyes project
// described above and using the same license. 
// Added the distance trigger into the config.h file.
// Also made it possible to change the eye type on the fly.
//--------------------------------------------------------------------------
...
// setup the sonar
UltraSonicDistanceSensor ds(0, 1);
...
#define TRACKING
uint8_t scleraWidth = SCLERA_WIDTH_DEFAULT;
uint8_t screenWidth = SCREEN_WIDTH_DEFAULT;
uint8_t scleraHeight = SCLERA_HEIGHT_DEFAULT;
uint8_t screenHeight = SCREEN_HEIGHT_DEFAULT;
uint8_t irisMapHeight = IRIS_MAP_HEIGHT_DEFAULT;
uint8_t irisHeight = IRIS_HEIGHT_DEFAULT;
uint8_t irisWidth = IRIS_WIDTH_DEFAULT;
uint16_t irisMapWidth = IRIS_MAP_WIDTH_DEFAULT;
uint8_t irisMin = IRIS_MIN_DEFAULT;
uint16_t irisMax = IRIS_MAX_DEFAULT;

const uint16_t (*scleraDef)[SCLERA_WIDTH_DEFAULT] = scleraDefault;
const uint8_t (*upperDef)[SCREEN_WIDTH_DEFAULT] = upperDefault;
const uint8_t (*lowerDef)[SCREEN_WIDTH_DEFAULT] = lowerDefault;
const uint16_t (*polarDef)[IRIS_WIDTH_DEFAULT] = polarDefault;
const uint16_t (*irisDef)[IRIS_MAP_WIDTH_DEFAULT] = irisDefault;

const uint16_t (*scleraDra)[SCLERA_WIDTH_DRAGON] = scleraDragon;
const uint8_t (*upperDra)[SCREEN_WIDTH_DRAGON] = upperDragon;
const uint8_t (*lowerDra)[SCREEN_WIDTH_DRAGON] = lowerDragon;
const uint16_t (*polarDra)[IRIS_WIDTH_DRAGON] = polarDragon;
const uint16_t (*irisDra)[IRIS_MAP_WIDTH_DRAGON] = irisDragon;
...
void drawEye( // Renders one eye.  Inputs must be pre-clipped & valid.
  ...
  uint16_t colors[screenWidth];



  uint8_t  irisThreshold = (128 * (1023 - iScale) + 512) / 1024;
  uint32_t irisScale     = irisMapHeight * 65536 / irisThreshold;
...
if (eyeType == 1) {
  for (screenY = 0; screenY < screenHeight; screenY++, scleraY++, irisY++) {
    ...
    for (screenX = 0; screenX < screenWidth; screenX++, scleraX++, irisX++) {
      if ((lowerDra[screenY][screenX] <= lT) ||
          (upperDra[screenY][screenX] <= uT)) {             // Covered by eyelid
        p = 0;
      } else if ((irisY < 0) || (irisY >= irisHeight) ||
                 (irisX < 0) || (irisX >= irisWidth)) { // In sclera
        p = scleraDra[scleraY][scleraX];
      } else {                                          // Maybe iris...
        p = polarDra[irisY][irisX];                        // Polar angle/dist
        ...
        if (d < irisThreshold) {                        // Within scaled iris area
          ...
          p = irisDra[d][a];                               // Pixel = iris
        } else {                                        // Not in iris
          p = scleraDra[scleraY][scleraX];                 // Pixel = sclera
        }
      }
    ...
    eye[e].display->writeRect(0, screenY, screenWidth, 1, colors);
  } // end scanline
} else {
  for (screenY = 0; screenY < screenHeight; screenY++, scleraY++, irisY++) {
    scleraX = scleraXsave;
    irisX   = scleraXsave - (scleraWidth - irisWidth) / 2;
    for (screenX = 0; screenX < screenWidth; screenX++, scleraX++, irisX++) {
      if ((lowerDef[screenY][screenX] <= lT) ||
          (upperDef[screenY][screenX] <= uT)) {             // Covered by eyelid
        p = 0;
      } else if ((irisY < 0) || (irisY >= irisHeight) ||
                 (irisX < 0) || (irisX >= irisWidth)) { // In sclera
        p = scleraDef[scleraY][scleraX];
      } else {                                          // Maybe iris...
        p = polarDef[irisY][irisX];                        // Polar angle/dist
        d = p & 0x7F;                                   // Distance from edge (0-127)
        if (d < irisThreshold) {                        // Within scaled iris area
          d = d * irisScale / 65536;                    // d scaled to iris image height
          a = (irisMapWidth * (p >> 7)) / 512;        // Angle (X)
          p = irisDef[d][a];                               // Pixel = iris
        } else {                                        // Not in iris
          p = scleraDef[scleraY][scleraX];                 // Pixel = sclera
        }
      }
...
void frame( // Process motion for a single frame of left or right eye
  ...
  if (frameTimer <= 10) {
    ...
    if (ds.measureDistanceCm() < triggerDistance && ds.measureDistanceCm() >= 0) {
      tooClose = true;
    } else {
      tooClose = false;
    }
  ...
  if (!tooClose) {
   // #define TRACKING
    ...
    scleraWidth = SCLERA_WIDTH_DEFAULT;
    screenWidth = SCREEN_WIDTH_DEFAULT;
    scleraHeight = SCLERA_HEIGHT_DEFAULT;
    screenHeight = SCREEN_HEIGHT_DEFAULT;
    irisMapHeight = IRIS_MAP_HEIGHT_DEFAULT;
    irisHeight = IRIS_HEIGHT_DEFAULT;
    irisWidth = IRIS_WIDTH_DEFAULT;
    irisMapWidth = IRIS_MAP_WIDTH_DEFAULT;
    irisMin = IRIS_MIN_DEFAULT;
    irisMax = IRIS_MAX_DEFAULT;
    eyeType = 0;
    ...
    scleraWidth = SCLERA_WIDTH_DRAGON;
    screenWidth = SCREEN_WIDTH_DRAGON;
    scleraHeight = SCLERA_HEIGHT_DRAGON;
    screenHeight = SCREEN_HEIGHT_DRAGON;
    irisMapHeight = IRIS_MAP_HEIGHT_DRAGON;
    irisHeight = IRIS_HEIGHT_DRAGON;
    irisWidth = IRIS_WIDTH_DRAGON;
    irisMapWidth = IRIS_MAP_WIDTH_DRAGON;
    irisMin = IRIS_MIN_DRAGON;
    irisMax = IRIS_MAX_DRAGON;    
    eyeType = 1;
  ...
  eyeX = map(eyeX, 0, 1023, 0, scleraWidth  - 128);
  eyeY = map(eyeY, 0, 1023, 0, scleraHeight - 128);
  if (eyeIndex == 1) eyeX = (scleraWidth - 128) - eyeX; // Mirrored display
  ...
  if (eyeX > (scleraWidth - 128)) eyeX = (scleraWidth - 128);
  ...
  if (sampleY < 0) n = 0;
  else            n = (upperDef[sampleY][sampleX] +
                         upperDef[sampleY][screenWidth - 1 - sampleX]) / 2;
  ...
  drawEye(eyeIndex, iScale, eyeX, eyeY, n, lThreshold, eyeType);
}
...
uint16_t oldIris = (irisMin + irisMax) / 2, newIris;
